...
def train(mnist):
    ...
    reshaped_xs=tf.reshape(x, [BATCH_SIZE, mnist_inference.IMAGE_SIZE, mnist_inference.IMAGE_SIZE, mnist_inference.NUM_CHANNELS])
    #y_=tf.placeholder(tf.float32, shape=[None, mnist_inference.OUTPUT_NODE],name="y-input")
    y_=tf.placeholder(tf.float32, shape=[None, mnist_inference.OUTPUT_NODE],name="y-input")
    regularizer=tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)
    y=mnist_inference.inference(reshaped_xs,True, regularizer)
